...
function PreviewWrapper({ store }: { store: IMainStore }) {
  ...
  function renderAside() {
    ...
    return (
      <AsideNav
        ...
        renderLink={({ link, toggleExpand, classnames: cx, depth }: any) => {
          ...
          children.push(
            <i
              ...
              onClick={(e: React.MouseEvent) => {
                e.preventDefault()
                navigate(`/edit/${paths.indexOf(link.path)}`)
              }}
            />
          )
          ...
          return link.path ? (
            link.active ? (
              ...
              <Link
                to={link.path[0] === '/' ? link.path : `${link.path}`}
                onClick={(e) => {
                  // 阻止默认行为,使用 React Router 导航
                  e.preventDefault()
                  navigate(link.path)
                }}
              >
        ...
        isActive={(link: any) => isActive(link.path && link.path[0] === '/' ? link.path : `${link.path}`)}
      />
    )
  }
...
export default inject('store')(observer(PreviewWrapper))
